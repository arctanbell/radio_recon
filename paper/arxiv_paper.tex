...
\begin{abstract}
	...
	Matched same-backbone ablations further show that removing SD input causes a 12.9--14.0\,dB PSNR drop, indicating that low-spatial-frequency information is a major contributor to the final image quality.
...
The comparison separates classical CLEAN, input-only references, dirty-only learned restoration, and SD-inclusive radio models, and it motivates the dedicated same-backbone SD study below.
...
Because the benchmark varied input modality, model design, and training objective simultaneously, the specific contribution of SD was isolated separately through the matched-backbone ablation below.

\subsection{Strict SD Training Ablation}

A controlled same-backbone SD ablation is further performed by training each radio architecture in two modes: \textit{with SD} (PSF+dirty+SD) and \textit{PSF+dirty only}.
...
\begin{table*}[htbp]
	\centering
	\caption{Strict SD training ablation on identical backbones (1,272 evaluation samples).
...
\begin{figure}[htbp]
	...
	\caption{PSNR gain from SD input under strict same-backbone training ablation.
		All three backbones show large improvements when SD is included.
	}
	\label{fig:sd_train_delta}
\end{figure}

The same-backbone ablation indicates that SD input is a major contributor to reconstruction quality in this task.
...
The strict same-backbone ablation indicates that the SD channel is not merely correlated with stronger model configurations; it is a major source of performance gain under the current benchmark.
...
\begin{enumerate}
	...
	\item Same-backbone ablations show that the auxiliary SD channel provides strong complementary short-spacing information, with a 12.9--14.0\,dB PSNR penalty when it is removed.
...
Code, configuration files, evaluation scripts, and sample-list metadata for reproducing the main comparisons are available at \url{https://github.com/arctanbell/radio_recon.git}.
...
To improve reproducibility, the principal comparisons are tied to fixed sample-name lists, explicit normalization rules, configuration-recorded loss weights, and checkpoint-based evaluation.
